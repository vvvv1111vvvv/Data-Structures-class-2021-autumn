...
2. 문제 : 실습 11.3: 11.3절의 kruskal의 최소비용신장트리코드에서 가중치가 가장 작은 간선을 뽑기 위해 간선을 가중치의 내림차순으로
        정렬하였다. 이 코드를 힙을 사용하는 방법으로 수정하라, 가중치가 가장 작은 간선을 뽑아야하기 때문에 최소 힙을 사용해야 한다
        파이썬의 heapq모듈을 사용해도 좋다.
...
#2. Kruskal의 최소신장트리 알고리즘함수 MSTKruskal()를 호출한다.
    ...
    #2-2. 모든간선을 간선리스트에 추가 (튜플로 간선과 가중치를 heapq모듈의 heappush 메소드를 이용해 eList에 저장, (i,j 는 인덱스))
    ...
    #2-5. 간선의 개수가 정점의 개수보다 1 작을 동안 반복
        #2-5-1.  가중치가 가장 작은 간선의 두 정점이 속한 집합을 비교한다. (가중치, 정점u의 인덱스, 정점v의 인덱스) find()호출
...
import heapq   #보통 리스트를 마치 최소 힙처럼 다룰 수 있도록 도와준다.
                #리스트와 별개의 자료구조가 아님
...
def MSTKruskal(vertex,adj):
    ...
    while (edgeAccepted < vsize-1):
        e=heapq.heappop(eList)    # e : 가중치가 가장 작은 간선 (가중치, 정점u의 인덱스, 정점v의 인덱스)
        uset = find(e[1])       #두 정점이 속한 집합번호
        vset  = find(e[2])
        if uset != vset:        #두 정점이 속한 집합이 다르면
            print("간선추가: (%s,%s,%d)" %          #간선추가 (vertex1,vertex2, 가중치) 출력
                 (vertex[e[1]],vertex[e[2]],e[0]))
            sum+=e[0]
